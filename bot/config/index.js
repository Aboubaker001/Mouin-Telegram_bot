...
export function setupBot(bot) {
  ...
  bot.command('ابدأ', start);
  bot.command('تحقق', verify);
  bot.command('الجدول', schedule);
  bot.command('المحتوى', content);
  bot.command('أسئلة', faq);
  bot.command('اختبار', quiz);
  bot.command('إحصائيات', stats);
  bot.command('نشر', publish);
  bot.command('تذكير', reminder);
  ...
  bot.command('مساعدة', (ctx) => ctx.reply(
    "📚 *قائمة الأوامر المتاحة*\n\n" +
    "👋 *أوامر أساسية:*\n" +
    "/ابدأ - بدء استخدام البوت والتسجيل\n" +
    "/تحقق [رمز] - التحقق من الهوية\n" +
